...
const Sidebar = ({expand, setExpand}) => {
    ...
    const {user, chats, createNewChat, isLoading} = useAppContext()
    ...
    <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
      <div>
        ...
        <button 
            ...
            className={`mt-8 flex items-center justify-center cursor-pointer ${isLoading ? 'opacity-50' : ''} ${expand ? "bg-primary hover:opacity-90 rounded-2xl gap-2 p-2.5 w-max" : "group relative h-9 w-9 mx-auto hover:bg-gray-500/30 rounded-lg"}`}
            disabled={isLoading}
        >
            ...
            {expand && <p className='text-white text font-medium'>{isLoading ? 'Creating...' : 'New chat'}</p>}
        </button>
        ...
        <div className={`mt-8 text-white/25 text-sm ${expand ? "block" : "hidden"}`}>
            ...
            {chats.length > 0 ? (
                ...
                <p className="text-gray-400 italic text-xs mt-2">No chats available</p>
            )}
        </div>
      </div>
